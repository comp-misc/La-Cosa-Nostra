...
export = (asset_name: string): Discord.Attachment => {
		...
		if (flavour) {
			...
			for (let i = 0; i < swaps.length; i++) {
				if (swaps[i].from === asset_name) {
					const attachment = new Discord.Attachment(flavour.assets[swaps[i].to], swaps[i].to)
					return attachment
				}
			}
		}
	...
	const attachment = new Discord.Attachment(assets[asset_name], asset_name)
	return attachment
}
