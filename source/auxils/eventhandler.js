		...
		if (count > ping_config["threshold"]) {
			message.channel.send(
				":x: <@" +
					...
					"__ time" +
					pettyFormat("s", ping_config["threshold"]) +
					" in the timespan of **" +
		...
		if (delta / edit_config["minimum-character-count"] >= edit_config["edit-ratio"]) {
			...
			new_message.channel.send(
				":x: <@" +
					...
					"__ character" +
					pettyFormat("s", old_message.content.length) +
					" → __" +
					new_message.content.length +
					"__ character" +
					pettyFormat("s", new_message.content.length) +
					"] by __" +
		...
		if (length >= deletion_config["minimum-character-count"]) {
			message.channel.send(
				":x: <@" +
					...
					length +
					" character" +
					pettyFormat("s", length) +
					"] posted on **" +
					formatUTCDate(message.createdAt.getTime()) +
