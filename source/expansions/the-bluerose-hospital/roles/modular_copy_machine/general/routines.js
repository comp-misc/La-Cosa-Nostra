...
module.exports = function (player) {
	...
	var powers = player.attributes.filter(
		(x) => x.attribute.modular && x.attribute["modular-details"]["cluster"] === "power"
	)
	...
	if (powers.length > 0) {
		...
		player.game.sendPeriodPin(
			channel,
			":dvd: You currently have the following modular powers:\n\n```fix\n" +
				powers.join("\n") +
	...
	channel.send(
		":zap: You still need to execute **" +
			(3 - success) +
			"** action" +
			auxils.vocab("s", 3 - success) +
			" to win this game."
	)
}
