...
module.exports = function (player) {
	...
	for (var i = 0; i < followers.length; i++) {
		var blessings = followers[i].attributes.filter(
			(x) => x.attribute.modular && x.attribute["modular-details"].cluster === "blessing"
		)
	...
	player
		.getPrivateChannel()
		.send(
			":pray: You see that there are **" +
				followers.length +
				"** followers and they have the following blessings:\n\n```fix\n" +
				blessings_map.join("\n") +
				"\n```"
		)

	var curses = player.attributes.filter(
		(x) => x.attribute.modular && x.attribute["modular-details"]["cluster"] === "curse"
	)
	...
	if (curses.length > 0) {
		...
		game.sendPeriodPin(
			channel,
			":candle: You currently have the following curses:\n\n```fix\n" +
				curses.join("\n") +
