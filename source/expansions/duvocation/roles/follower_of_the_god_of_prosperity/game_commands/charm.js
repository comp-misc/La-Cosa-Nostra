...
module.exports = function (game, message, params) {
	...
	if (params[0] === undefined) {
		message.channel.send(
			":x: Wrong syntax! Please use `" + config["command-prefix"] + "charm <alphabet/username/nobody>` instead!"
		)
		return null
	}
	...
	message.channel.send(
		":four_leaf_clover: You have decided to give charms to " +
			auxils.pettyFormat(names.map((x) => "**" + x + "**")) +
			" tonight."
	)
}
