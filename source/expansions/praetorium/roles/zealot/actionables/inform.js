	...
	for (var i = 0; i < praetors.length; i++) {
		...
		if (log.successful) {
			...
			if (visitor_names.length > 0) {
				game.addMessage(
					":exclamation: Praetor **" +
						praetor.getDisplayName() +
						"**'s conversion was unsuccessful. Last night, " +
						auxils.pettyFormat(visitor_names) +
						" visited the praetor's target."
				)
			} else {
				game.addMessage(
					":exclamation: Praetor **" +
						praetor.getDisplayName() +
						"**'s conversion was unsuccessful. Last night, nobody visited the praetor's target."
				)
			}
		}
	}
