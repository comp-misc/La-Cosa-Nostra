...
const shoot: RoleCommand = async (game, message, params, from) => {
	const role = from.role.role as TownMarksman
	...
	const config = (from.role.role as TownMarksman).config
	...
	await game.addAction("town_marksman/shoot", ["instant"], {
		expiry: 1,
		from,
		to: actualTarget,
