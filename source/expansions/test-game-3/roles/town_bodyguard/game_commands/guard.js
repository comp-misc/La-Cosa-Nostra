...
module.exports = function (game, message, params) {
	...
	if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
		actions.delete((x) => x.from === from.identifier && x.identifier === "town_bodyguard/guard")

		message.channel.send(":shield: You have decided not to guard anyone tonight.")
		return null
	}
	...
	if (to.id === message.author.id) {
		...
		game.addAction("town_bodyguard/guard", ["cycle"], {
	...
	message.channel.send(":shield: You have decided to guard **" + mention + "** tonight.")
}
