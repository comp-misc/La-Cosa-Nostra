...
module.exports = function (game, message, params) {
	...
	if (params[0] === undefined) {
		message.channel.send(
			":x: Wrong syntax! Please use `" + config["command-prefix"] + "infect <alphabet/username/nobody>` instead!"
		)
		return null
	}
	...
	actions.delete((x) => x.from === from.identifier && x.identifier === "plaguebearer/infect")
	...
	if (to.id === message.author.id) {
		...
		game.addAction("plaguebearer/infect", ["cycle"], {
			name: "Plaguebearer-infect",
			expiry: 1,
