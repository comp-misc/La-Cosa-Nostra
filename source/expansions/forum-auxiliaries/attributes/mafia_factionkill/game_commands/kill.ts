...
const kill: TargetRoleCommand = async (game, message, target, from) => {
	...
	if (target === "nobody") {
		await message.reply(":no_entry: You have decided not to use the factional kill.")
		await game
			.getChannel("mafia")
			.send(
				":exclamation: **" +
	...
	await message.reply(
		":no_entry: You have decided to use the factional kill on **" + target.getDisplayName() + "** tonight."
	)
	await game
		.getChannel("mafia")
		.send(
