...
module.exports = function (game, message, params) {
	...
	if (params[0] === undefined) {
		message.channel.send(
			":x: Wrong syntax! Please use `" + config["command-prefix"] + "deliver <alphabet/username/nobody>` instead!"
		)
		return null
	}
	...
	actions.delete(
		(x) =>
			...
			(x.identifier === "odd_night_fence/delivery" || x.identifier === "odd_night_fence/random_delivery")
	)

	if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
		message.channel.send(
			":mailbox_closed: You have decided not to deliver a gun to anyone. A random player will be chosen instead."
		)
	...
	if (from.misc.fence_log[0] === to.identifier) {
		message.channel.send(
			":x: You may not deliver a gun to the same person consecutively! Please re-pick your target or a random player will be chosen instead!"
		)
