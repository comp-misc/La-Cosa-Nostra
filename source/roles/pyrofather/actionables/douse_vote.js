...
module.exports = function (actionable, game, params) {
  ...
  because of a role assignation problem.*/
	...
	var pyromaniac = game.find(
		(x) => x.role_identifier === "pyromaniac" && x.isAlive() && !x.getStatus("roleblocked") && !x.getStatus("kidnapped")
	)
	...
	if (pyromaniac) {
		...
		actions.delete(
			(x) =>
				x.from === pyromaniac.identifier &&
				(x.identifier === "pyromaniac/douse" || x.identifier === "pyromaniac/ignite")
		)
		...
		game.addMessage(
			pyromaniac,
			":exclamation: The __Pyrofather__ has ordered you to douse his target, **" + to.getDisplayName() + "**."
		)
	} else {
