...
module.exports = function () {
			...
			if (is_subprocess) {
				...
				try {
					...
					if (!data.directories["expansions"]) {
						...
						if (has_default) {
							process.directories["expansions"] = process.directories["expansions"].concat(
								data.directories["expansions"].filter((x) => x === "default")
							)
						} else {
	...
	if (version.engineStrict) {
		...
		if (!version_comparison) {
			var error = new Error(
				"Failed to initialise due to incompatible Node version. You are using version " +
