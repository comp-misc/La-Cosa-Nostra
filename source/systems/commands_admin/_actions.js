...
module.exports = async function (message, params, config) {
	...
	for (var i = 0; i < params.length; i++) {
		...
		if (key === "-*") {
			...
			actions.forEach((x) =>
				x.action.tags.includes(check) || x.action.triggers.includes(check) || x.action.identifier.includes(check)
					? (x.used = false)
					: null
			)
		} else {
			actions.forEach((x) =>
				x.action.tags.includes(key) || x.action.triggers.includes(key) || x.action.identifier.includes(key)
					? (x.used = true)
					: null
			)
		}
