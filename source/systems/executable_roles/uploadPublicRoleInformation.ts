...
const uploadPublicRoleInformation = async (game: Game, roles: Player[]): Promise<void> => {
	...
	for (let i = 0; i < roles.length; i++) {
		...
		info += cpl(role.expandedRole().alignment)

		if (flavour.info["show-role-category"]) {
			info += "-" + cpl(role.expandedRole().class)
		}

		sendable = sendable.replace(/{;info}/g, info)
		sendable = sendable.replace(/{;description}/g, flavour_role["description"] || role.expandedRole().description)
