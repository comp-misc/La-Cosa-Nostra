...
module.exports = async function (game, message, params) {
    ...
    if (players_dead < 1) {
        ...
        for (var i = 0; i < sorted_list.length; i++) {
            ...
            display_message = display_message + returnDayNight(information_list[i][0]) + ": " + sorted_list[i][1].getDisplayName() + " - " + sorted_list[i][1].getDisplayRole(false) + "\n";

        };

        message.channel.send("There " + grammar + " __" + players_dead + "__ dead player" + auxils.vocab("s", players_dead) + ":\n```" + display_message + "```");        
    };

    function returnDayNight(number) {

        if ((Math.round((number % 2)*10)/10) % 2 == 1 || (Math.round((number % 2)*10)/10) % 2 == 1.1 || (Math.round((number % 2)*100)/100) % 2 == 1.05) {
