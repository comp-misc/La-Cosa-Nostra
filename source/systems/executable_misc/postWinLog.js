...
module.exports = async function (game, faction, description) {
    ...
    for (var i = 0; i < players.length*2; i++) {
      ...
      for(var j = 0; j < sorted_list.length; j++){
        ...
        if(Math.floor(information_list[j][0]) == i) {
          display_message = display_message + returnDayNight(information_list[j][0]) + ": " + sorted_list[j][1].getDisplayName() + returnCauseOfDeath(information_list[j][0]) + "\n";
          count++;
        };
      };
  ...
  function returnDayNight(number) {
    ...
    if ((Math.round((number % 2)*10)/10) == 1 || (Math.round((number % 2)*10)/10) == 1.1 || (Math.round((number % 2)*100)/100) == 1.05) {
