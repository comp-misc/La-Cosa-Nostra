...
for (var role in roles) {
	...
	for (var i = 0; i < commands.length; i++) {
		...
		main.role[key].properties.push({ command: key, role: role, runnable: require(commands[i]) })
	}
}
...
for (var attribute in attributes) {
	...
	for (var i = 0; i < commands.length; i++) {
		...
		main.role[key].properties.push({ command: key, attribute: attribute, runnable: require(commands[i]) })
	}
}
