...
module.exports = class {
    ...
    */
		...
		// Checksum
		var checksum = auxils.hash(
			auxils.hash(JSON.stringify(savable, auxils.jsonInfinityCensor), "md5") +
				auxils.hash(savable.last_save_date.getTime().toString(), "md5"),
			"md5"
		)
		...
		// Save object
		fs.writeFileSync(
			data_directory + "/game_cache/game.save",
			encode(JSON.stringify(savable, auxils.jsonInfinityCensor))
		)
		...
		for (var i = 0; i < players.length; i++) {
			...
			// Checksum
			var checksum = auxils.hash(
				auxils.hash(JSON.stringify(player, auxils.jsonInfinityCensor), "md5") +
					auxils.hash(player.last_save_date.getTime().toString(), "md5"),
				"md5"
			)
...
module.exports.load = function (client, config) {
	...
	if (
		checksum !==
		auxils.hash(
			auxils.hash(JSON.stringify(save, auxils.jsonInfinityCensor), "md5") +
				auxils.hash(new Date(save.last_save_date).getTime().toString(), "md5")
		)
	) {
	...
	for (var i = 0; i < player_saves.length; i++) {
		...
		if (
			checksum !==
			auxils.hash(
				auxils.hash(JSON.stringify(player_save, auxils.jsonInfinityCensor), "md5") +
					auxils.hash(new Date(player_save.last_save_date).getTime().toString(), "md5")
			)
		) {
