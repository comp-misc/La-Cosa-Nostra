...
module.exports = function (game, message, params) {
	...
	actions.delete((x) => x.from === from.identifier && x.identifier === "mafia_rolecop/check")

	if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
		message.channel.send(":mag: You have decided not to check anyone tonight.")
		game.getChannel("mafia").send(":exclamation: **" + from.getDisplayName() + "** is not checking anybody tonight.")
		return null
	}
