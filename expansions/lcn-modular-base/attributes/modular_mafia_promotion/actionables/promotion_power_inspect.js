...
module.exports = function (actionable, game, params) {
	...
	game.addMessage(
		player,
		":exclamation: You have obtained infinite uses of the Power Inspect power as no members of the Mafia carry it." +
			addendum
	)
	...
	function hasPower(player, power, uses) {
		...
		if (uses) {
			return attributes.some(
				(x) =>
					x.attribute.modular &&
					x.attribute.name.toLowerCase() === power.toLowerCase() &&
					x.attribute["modular-details"]["cluster"].toLowerCase() === "power" &&
					x.tags.uses === uses
			)
		} else {
			return attributes.some(
