...
module.exports = function (game, message, params) {
	...
	if (params[0] === undefined) {
		message.channel.send(
			":x: Wrong syntax! Please use `" + config["command-prefix"] + "heal <alphabet/username/nobody>` instead!"
		)
		return null
	}
	...
	actions.delete(
		(x) =>
			x.from === from.identifier && (x.identifier === "quack/doc_protect" || x.identifier === "quack/doc_protect_self")
	)
	...
	if (to.id === message.author.id) {
		...
		game.addAction("quack/doc_protect_self", ["cycle"], {
			name: "Doc-protect",
			expiry: 1,
		...
		game.addAction("quack/doc_protect", ["cycle"], {
			name: "Doc-protect",
			expiry: 1,
			from: message.author.id,
			to: to.id,
