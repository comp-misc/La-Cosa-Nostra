...
module.exports = function (game, message, params) {
	...
	if (game.getPeriod() % 4 !== 0) {
		message.channel.send(":x:  You may only interrogate on even days!")
	...
	if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
		actions.delete((x) => x.from === from.identifier && x.identifier === "town_even_day_interrogator/interrogate")
	...
	if (to.id === message.author.id) {
		...
		game.addAction("town_even_day_interrogator/interrogate", ["cycle"], {
