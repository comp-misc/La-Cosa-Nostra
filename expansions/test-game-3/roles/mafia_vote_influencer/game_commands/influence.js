...
module.exports = function (game, message, params) {
  ...
  if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
    ...
    actions.delete(x => x.from === from.identifier && (x.identifier === "mafia_vote_influencer/influence" || x.identifier === "mafia_vote_influencer/block" || x.identifier === "mafia_vote_influencer/no_action"));
    
    message.channel.send(":ballot_box: You have decided not to block nor influence the vote of anyone tonight.");
    game.getChannel("mafia").send(":exclamation: **" + from.getDisplayName() + "** has decided not to block nor influence the vote of anyone tonight.");
  ...
  game.getChannel("mafia").send(":exclamation: **" + from.getDisplayName() + "** has decided to influence the vote of **" + mention + "** tonight.");
