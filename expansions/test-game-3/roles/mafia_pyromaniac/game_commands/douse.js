...
module.exports = function (game, message, params) {
  ...
  if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
    ...
    actions.delete(x => x.from === from.identifier && (x.identifier === "mafia_pyromaniac/douse" || x.identifier === "mafia_pyromaniac/ignite"));

    message.channel.send(":oil: You have decided not to douse nor ignite tonight.");
    game.getChannel("mafia").send(":exclamation: **" + from.getDisplayName() + "** has decided not to douse nor ignite anyone tonight.");
    return null;
  };
  ...
  game.getChannel("mafia").send(":exclamation: **" + from.getDisplayName() + "** has decided to douse **" + mention + "** tonight.");
