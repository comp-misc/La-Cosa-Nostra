...
module.exports = function (game, message, params) {
  ...
  if (params[0] === undefined) {
    message.channel.send(":x:  Wrong syntax! Please use `" + config["command-prefix"] + "execute <alphabet/username/nobody>` instead!");
    return null;
  };
  ...
  if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
    message.channel.send(":crossed_swords:  You have now selected to execute anyone today.");
    return null;
  };
  ...
  if (to.id === message.author.id) {
    ...
    game.addAction("town_president/execute", ["instant"], {
      name: "GodofWar-kill",
      expiry: 1,
