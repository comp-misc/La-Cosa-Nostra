...
module.exports = function (game, message, params) {
	...
	actions.delete((x) => x.from === from.identifier && x.identifier === "mafia_vigilante/shoot")

	if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
		message.channel.send(":gun: You have decided not to attack anyone tonight.")
		return null
	}
	...
	if (to.id === message.author.id) {
		...
		game.addAction("mafia_vigilante/shoot", ["cycle"], {
			name: "mafia_vigilante-shoot",
			expiry: 1,
