...
module.exports = function (game, message, params) {
	...
	if (!already_alerting) {
		message.channel.send(
			":x: You have already decided not to go on alert tonight. Use `" +
				config["command-prefix"] +
				"alert` to choose to go on alert!"
		)
		return null
	}

	actions.delete((x) => x.from === from.identifier && x.identifier === "town_veteran/alert")
