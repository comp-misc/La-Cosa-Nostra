...
module.exports = function (game, message, params) {
	...
	var forged = game.findAll(
		(x) => x.isAlive() && x.misc.forgery === game.getStep() && x.misc.forger === from.identifier
	)
	...
	if (params.length < 1 || !params[0]) {
		message.channel.send(
			":x: Wrong syntax! Please use `" +
				...
				'forge <"retract"/alphabet/name/nobody> <alphabet/name>` instead!'
		)
		return null
	}
	...
	if (params[0].toLowerCase() === "retract") {
		...
		message.channel.send(
			":pencil: You have retracted the forged vote" +
				auxils.vocab("s", forged_names.length) +
				" from " +
				forged_names +
				"."
		)
