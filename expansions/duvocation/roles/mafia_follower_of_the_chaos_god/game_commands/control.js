...
module.exports = function (game, message, params) {
	...
	if (params.length < 2 || !params[0] || !params[1]) {
		message.channel.send(
			":x: Wrong syntax! Please use `" +
				...
				"control <alphabet/name/nobody> <alphabet/name>` instead!"
		)
		return null
	}
	...
	message.channel.send(
		":dart: You have decided to redirect all actions from **" + p1_name + "** to **" + p2_name + "** tonight."
	)
	game
		.getChannel("mafia")
		.send(
			":exclamation: **" +
				from.getDisplayName() +
				"** is redirecting all actions from **" +
				p1_name +
				"** to **" +
				p2_name +
				"** tonight."
