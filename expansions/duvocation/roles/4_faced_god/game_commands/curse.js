...
for (var i = 0; i < keys.length; i++) {
	...
	if (attribute.modular && attribute["modular-details"]["cluster"] === "curse") {
		curses.push(attribute)
	}
}
...
module.exports = function (game, message, params) {
	...
	if (params.length < 1) {
		message.channel.send("**List of available curses:**\n```fix\n" + curses.map((x) => x.name).join(", ") + "```")
		return null
	}
	...
	// Find
	var modular = curses.find((x) => x.name.toLowerCase() === check.toLowerCase())
	...
	var sendable = "Modular curse: **" + modular.name + "**"
