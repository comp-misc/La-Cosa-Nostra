...
module.exports = async function (game, message, params) {
	...
	if (params.length < 1) {
		await message.channel.send(
			":x: Wrong syntax! Use `" + config["command-prefix"] + "cc <create/add/remove>` instead!"
		)
		return null
	}
	...
	switch (params[0].toLowerCase()) {
		case "create":
			...
			if (params.length < 3) {
				await message.channel.send(
					":x: Wrong syntax! Use `" +
						...
						"cc create <name of channel> [<alphabet/name>...]` instead!"
				)
				return null
			}
			...
			if (
				...
				game.conspiracy_channels.channels.filter((x) => x.owner === sender.identifier).length >
					expansion_config["max-ccs-per-player"]
			) {
				await message.channel.send(
					":x: You may not create more than **" + expansion_config["max-ccs-per-player"] + "** conspiracy channels."
				)
				return null
			}
			...
			for (var i = 2; i < params.length; i++) {
				...
				if (!player.status.alive) {
					await message.channel.send(
						":x: **" + player.getDisplayName() + "** is dead! You cannot create conspiracy channels with dead players!"
					)
					return null
				}
			...
			if (players.length > expansion_config["max-players-per-cc"] - 1) {
				await message.channel.send(
					":x: A conspiracy channel cannot exceed **" +
						expansion_config["max-players-per-cc"] +
		...
		case "add":
			...
			if (params.length < 2) {
				await message.channel.send(
					":x: Wrong syntax! Use `" + config["command-prefix"] + "cc add [<alphabet/name>...]` instead!"
				)
				return null
			}
			...
			await message.channel.send(
				":exclamation: Added " +
					auxils.pettyFormat(players.map((x) => "<@" + x.id + ">")) +
					" to the conspiracy channel <#" +
					message.channel.id +
					">."
			)
		...
		case "remove":
			...
			for (var i = 1; i < params.length; i++) {
				...
				if (!player.status.alive) {
					await message.channel.send(
						":x: **" + player.getDisplayName() + "** is dead! No need to remove them from your conspiracy channel!"
					)
					return null
				}
			...
			await message.channel.send(
				":exclamation: Removed " +
					auxils.pettyFormat(players.map((x) => "**" + x.getDisplayName() + "**")) +
	...
	async function createCC(name, owner, players) {
		...
		if (empty_category === null) {
			...
			empty_category = await guild.createChannel(category_name, {
				type: "category",
		...
		var channel = await guild.createChannel(name, {
			type: "text",
			permissionOverwrites: [{ id: guild.id, deny: ["READ_MESSAGES"] }],
		...
		var permissions = standard_permissions
			.concat(
				players.map((x) => {
					var target = message.client.users.get(x.id)
		...
		game.conspiracy_channels.channels.push({
			id: channel.id,
			owner: owner.identifier,
			members: players.map((x) => x.identifier),
		})
		...
		await channel.send(
			"**" +
				...
				"** has created a conspiracy channel <#" +
				channel.id +
				">.\n\n**Members**:\n" +
