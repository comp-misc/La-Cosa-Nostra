...
for (var i = 0; i < keys.length; i++) {
	...
	if (attribute.modular && attribute["modular-details"]["cluster"] === "ability") {
...
module.exports = function (message, params, config) {
	if (params.length < 1) {
		message.channel.send("**List of available abilities:**\n```fix\n" + powers.map((x) => x.name).join(", ") + "```")
		return null
	}
	...
	var sendable = "Modular Ability: **" + modular.name + "**"
