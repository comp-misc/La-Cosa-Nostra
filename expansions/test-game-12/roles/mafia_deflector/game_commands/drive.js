...
module.exports = function (game, message, params) {
  ...
  if (player1.score < 0.7 || params[0].toLowerCase() === "nobody") {
    ...
    game.getChannel("mafia").send(":exclamation: **" + from.getDisplayName() + "** has decided not to deflect actions performed on someone to anyone else tonight.");
    return null;
  };
  ...
  message.channel.send(":bus: You have decided to deflect actions performed on **" + p1_name + "** to **" + p2_name + "** tonight.");
  game.getChannel("mafia").send(":exclamation: **" + from.getDisplayName() + "** has decided to deflect action performed on **" + p1_name + "** to **" + p2_name + "** tonight.");
