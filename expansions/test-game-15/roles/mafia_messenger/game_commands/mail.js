...
module.exports = function (game, message, params) {
  ...
  if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
    ...
    actions.delete(x => x.from === from.identifier && x.identifier === "mafia_messenger/mail");
    
    message.channel.send(":mailbox_with_mail: You have decided not to send anyone a message tonight.");
    game.getChannel("mafia").send(":exclamation: **" + from.getDisplayName() + "** has decided not to message anyone tonight.");
    return null;
  };
  ...
  if (to.id === message.author.id) {
    ...
    game.addAction("mafia_messenger/mail", ["cycle"], {
      name: "Mailman-mail",
      expiry: 1,
  ...
  message.channel.send(":mailbox_with_mail: You have decided send **" + mention + "** the following anonymous message:\n```" + send + "```");
  game.getChannel("mafia").send(":exclamation: **" + from.getDisplayName() + "** has decided to send" + mention + "the following anonymous message:\n```" + send + "```");
};
