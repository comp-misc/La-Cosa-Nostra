...
module.exports = function (game, message, params) {
	...
	if (from.misc.doctor_protects_left < 1) {
		message.channel.send(":x:  You have no uses left!")
		return null
	}

	if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
		actions.delete((x) => x.from === from.identifier && x.identifier === "mafia_1_shot_doctor/protect")
	...
	game.addAction("mafia_1_shot_doctor/protect", ["cycle"], {
		name: "Doc-protect",
		expiry: 1,
