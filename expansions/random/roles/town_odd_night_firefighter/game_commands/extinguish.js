...
module.exports = function (game, message, params) {
  ...
  if (game.getPeriod() % 4 !== 1) {
    message.channel.send(":x:  You may only extinguish on odd nights!");
  ...
  if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
    ...
    actions.delete(x => x.from === from.identifier && x.identifier === "town_odd_night_firefighter/extinguish");
  ...
  if (to.id === message.author.id) {
    ...
    message.channel.send(":fire_extinguisher:  You have now selected to extinguish **" + to.getDisplayName() + "** tonight.")
  };
