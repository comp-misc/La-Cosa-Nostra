...
module.exports = function (player) {
  ...
  player.game.addAction("town_nonconsecutive_observer/no_action", ["cycle"], {
    name: "SE-no_action",
    expiry: 1,
    from: player,
    to: player
  ...
  if (player.misc.consecutive_night === false) {  
    player.game.sendPeriodPin(channel, ":telescope:  You may pick a player to observe tonight.\n\nUse `" + config["command-prefix"] + "observe <alphabet/name/nobody>` to select your target.");
  } else {
