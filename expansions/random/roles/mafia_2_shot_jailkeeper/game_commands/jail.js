...
module.exports = function (game, message, params) {
	...
	if (from.misc.jailkeeper_jails_left < 1) {
		message.channel.send(":x:  You have no uses left!")
		return null
	}

	if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
		actions.delete((x) => x.from === from.identifier && x.identifier === "mafia_2_shot_jailkeeper/jail")
	...
	if (to.id === message.author.id) {
		...
		game.addAction("mafia_2_shot_jailkeeper/jail", ["cycle"], {
