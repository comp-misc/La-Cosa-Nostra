...
module.exports = function (game, message, params) {
  ...
  if (game.getPeriod() % 4 !== 1) {
    message.channel.send(":x:  You may only watch a player on odd nights!");
  ...
  if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
    ...
    actions.delete(x => x.from === from.identifier && x.identifier === "town_odd_night_watcher/watch");
  ...
  if (to.id === message.author.id) {
    ...
    game.addAction("town_odd_night_watcher/watch", ["cycle"], {
      name: "Watcher-watch",
      expiry: 1,
