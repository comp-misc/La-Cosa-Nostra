...
module.exports = function (game, message, params) {
	...
	if (game.getPeriod() % 4 !== 1) {
		message.channel.send(":x:  You may only strongkill a player on odd nights!")
	...
	if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
		actions.delete((x) => x.from === from.identifier && x.identifier === "mafia_odd_night_strongman/strongkill")
	...
	if (to.id === message.author.id) {
		...
		game.addAction("mafia_odd_night_strongman/strongkill", ["cycle"], {
