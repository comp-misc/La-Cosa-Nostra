...
module.exports = function (game, message, params) {
	...
	if (from.misc.role_cop_investigates_left < 1) {
		message.channel.send(":x:  You have no uses left!")
		return null
	}

	if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
		actions.delete((x) => x.from === from.identifier && x.identifier === "town_2_shot_role_cop/check")
	...
	if (to.id === message.author.id) {
		...
		game.addAction("town_2_shot_role_cop/check", ["cycle"], {
			name: "Rolecop-check",
			expiry: 1,
