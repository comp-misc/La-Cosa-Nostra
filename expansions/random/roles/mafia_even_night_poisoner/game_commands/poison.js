...
module.exports = function (game, message, params) {
  ...
  if (game.getPeriod() % 4 !== 3) {
    message.channel.send(":x:  You may only poison a player on even nights!");
  ...
  if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
    ...
    actions.delete(x => x.from === from.identifier && x.identifier === "mafia_even_night_poisoner/poison");
  ...
  if (to.id === message.author.id) {
    ...
    game.addAction("mafia_even_night_poisoner/poison", ["cycle"], {
  ...
  game.getChannel("mafia").send(":test_tube:  **" + from.getDisplayName() + "** is poisoning **" + mention + "** tonight.");
