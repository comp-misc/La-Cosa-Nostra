...
module.exports = function (game, message, params) {
  ...
  if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
    ...
    actions.delete(x => x.from === from.identifier && (x.identifier === "town_nonconsecutive_tracker/track" || x.identifier === "town_nonconsecutive_tracker/no_action"));

    game.addAction("town_nonconsecutive_tracker/no_action", ["cycle"], {
      name: "SE-no_action",
      expiry: 1,
    ...
    message.channel.send(":mag_right:  You have now selected to not to track anyone tonight.");
    return null;
  };
  ...
  if (to.id === message.author.id) {
    ...
    game.addAction("town_nonconsecutive_tracker/track", ["cycle"], {
      name: "Tracker-track",
      expiry: 1,
      from: message.author.id,
      to: to.id
