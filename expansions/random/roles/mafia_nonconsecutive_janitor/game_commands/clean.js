...
module.exports = function (game, message, params) {
  ...
  if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
    ...
    actions.delete(x => (x.from === from.identifier && (x.identifier === "mafia_nonconsecutive_janitor/clean" || x.identifier === "mafia_nonconsecutive_janitor/no_action")));

    game.addAction("mafia_nonconsecutive_janitor/no_action", ["cycle"], {
      name: "SE-no_action",
      expiry: 1,
  ...
  game.addAction("mafia_nonconsecutive_janitor/clean", ["cycle"], {
    name: "Janitor-clean",
    expiry: 1,
    from: message.author.id,
    to: to.id
  ...
  game.getChannel("mafia").send(":drop_of_blood:  **" + from.getDisplayName() + "** is cleaning **" + mention + "** tonight. The factional kill needs to be used in addition to the clean action.");
