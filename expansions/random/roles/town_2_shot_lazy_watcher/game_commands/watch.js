...
module.exports = function (game, message, params) {
	...
	if (from.misc.lazy_watcher_watches_left < 1) {
		message.channel.send(":x:  You have no uses left!")
		return null
	}

	if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
		actions.delete((x) => x.from === from.identifier && x.identifier === "town_2_shot_lazy_watcher/watch")
	...
	if (to.id === message.author.id) {
		...
		game.addAction("town_2_shot_lazy_watcher/watch", ["cycle"], {
			name: "Watcher-watch",
			expiry: 1,
