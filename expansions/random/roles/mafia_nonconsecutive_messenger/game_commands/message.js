...
module.exports = function (game, message, params) {
  ...
  if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
    ...
    actions.delete(x => x.from === from.identifier && (x.identifier === "mafia_nonconsecutive_messenger/message" || x.identifier === "mafia_nonconsecutive_messenger/no_action"));
    
    game.addAction("mafia_nonconsecutive_messenger/no_action", ["cycle"], {
      name: "SE-no_action",
      expiry: 1,
  ...
  if (to.id === message.author.id) {
    ...
    game.addAction("mafia_nonconsecutive_messenger/message", ["cycle"], {
      name: "Mailman-mail",
      expiry: 1,
      from: message.author.id,
      to: to.id,
  ...
  message.channel.send(":envelope:  You have now selected to send **" + mention + "** the following anonymous message:\n```" + send + "```");
  game.getChannel("mafia").send(":envelope:  **" + from.getDisplayName() + "** is now sending **" + mention + "** the following anonymous message:\n```" + send + "```");
