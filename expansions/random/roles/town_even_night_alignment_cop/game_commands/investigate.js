...
module.exports = function (game, message, params) {
	...
	if (game.getPeriod() % 4 !== 3) {
		message.channel.send(":x:  You may only investigate on even nights!")
	...
	if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
		actions.delete((x) => x.from === from.identifier && x.identifier === "town_even_night_alignment_cop/investigate")
	...
	if (to.id === message.author.id) {
		...
		game.addAction("town_even_night_alignment_cop/investigate", ["cycle"], {
