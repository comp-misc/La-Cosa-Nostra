...
module.exports = function (game, message, params) {
  ...
  if (params[0] === undefined) {
    message.channel.send(":x:  Wrong syntax! Please use `" + config["command-prefix"] + "deflect <alphabet/name/nobody> <alphabet/name>` instead!");
    return null;
  };

  if (from.misc.deflector_deflections_left < 1) {
  ...
  if (player1.score < 0.7 || params[0].toLowerCase() === "nobody") {
    ...
    actions.delete(x => x.from === from.identifier && x.identifier === "mafia_1_shot_deflector/deflect");

    message.channel.send(":magnet:  You have now selected to not deflect actions from anyone tonight.");
    game.getChannel("mafia").send(":magnet:  **" + from.getDisplayName() + "** is not deflecting actions from anyone tonight.");
    return null;
  };
  ...
  game.addAction("mafia_1_shot_deflector/deflect", ["cycle"], {
    name: "Driver-drive",
    expiry: 1,
  ...
  message.channel.send(":magnet:  You have now selected to deflect actions performed on **" + p1_name + "** to **" + p2_name + "** tonight.");
  game.getChannel("mafia").send(":magnet:  **" + from.getDisplayName() + "** is deflecting actions performed on **" + p1_name + "** to **" + p2_name + "** tonight.");
