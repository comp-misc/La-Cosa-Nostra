...
module.exports = function (game, message, params) {
	...
	if (game.getPeriod() % 4 !== 3) {
		message.channel.send(":x:  You may only strongkill a player on even nights!")
	...
	if (params[0] === undefined) {
		message.channel.send(
			":x:  Wrong syntax! Please use `" + config["command-prefix"] + "strongkill <alphabet/username/nobody>` instead!"
		)
		return null
	}
	...
	if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
		actions.delete((x) => x.from === from.identifier && x.identifier === "mafia_even_night_strongman/strongkill")
	...
	if (to.id === message.author.id) {
		...
		game.addAction("mafia_even_night_strongman/strongkill", ["cycle"], {
