...
module.exports = function (game, message, params) {
  ...
  if (to.score < 0.7 || params[0].toLowerCase() === "nobody") {
    ...
    actions.delete(x => x.from === from.identifier && (x.identifier === "town_nonconsecutive_lazy_watcher/watch" || x.identifier === "town_nonconsecutive_lazy_watcher/no_action"));

    game.addAction("town_nonconsecutive_lazy_watcher/no_action", ["cycle"], {
      name: "SE-no_action",
      expiry: 1,
  ...
  if (to.id === message.author.id) {
    ...
    game.addAction("town_nonconsecutive_lazy_watcher/watch", ["cycle"], {
      name: "Watcher-watch",
      expiry: 1,
      from: message.author.id,
      to: to.id
